...
(define (generate p q e)
  "TODO: Implement"
  (append (list(list (* p q) e))
          (list (list (* p q) (mul-inv e (lcm (- p 1) (- q 1)))))
          )
  )
...
(define (mod-exp b e m)
  "TODO: Implement"
  (if (equal? e 0) 1
  (if (equal? (modulo e 2) 0) (modulo (expt (modulo (mod-exp b (/ e 2) m) m) 2) m)
      (modulo (* (modulo (mod-exp b (- e 1) m) m) b) m)
      )
  )
  )
...
(define (encrypt plaintext key)
  "TODO: Implement"
  (encryptH (string->list plaintext) key)
  )
(define (encryptH chars key)
(if (equal? chars '()) '()
(append (list (mod-exp (char->integer (car chars)) (car (cdr key)) (car key))) (encryptH (cdr chars) key))
    )
  )
...
(define (decrypt ciphertext key)
  "TODO: Implement"
  (list->string(decryptH ciphertext key))
  )

(define (decryptH ciphertext key)
  (if (equal? ciphertext '()) '()
(append (list(integer->char (mod-exp (car ciphertext) (car (cdr key)) (car key)))) (decryptH (cdr ciphertext) key)))
  )
