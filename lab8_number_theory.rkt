...
(define (divides? a b)
  "TODO: Implement"
  (if (equal? a 0) (if (equal? b 0) #t #f)
      (equal? 0 (remainder b a))
  ))
...
(define (waterjug cA cB g)
  "TODO: Implement"
  (if (wj-solvable? cA cB g)
  (waterjugH 0 0 cA cB g)
  #f
  ))

(define (waterjugH fA fB cA cB g)
(if (equal? fB g) (list(list fA fB))
(if (equal? fB cB) (append (list(list fA fB)) (waterjugH fA 0 cA cB g))
(if (equal? fA 0) (append (list(list fA fB)) (waterjugH cA fB cA cB g))
(if (> (+ fA fB) cB) (append (list(list fA fB)) (waterjugH (remainder (+ fA fB) cB) cB cA cB g))
(if (<= (+ fA fB) cB) (append (list(list fA fB)) (waterjugH 0 (+ fA fB) cA cB g))
#f)))))
)
...
(define (factorizeH n p)
  (if (>= p n) (list n)
  (if (divides? p n) (append (list p) (factorizeH (/ n p) 2))
      (factorizeH n (+ p 1))
  )))
...
(define (prime? n)
  "TODO: Implement"
  (if (equal? (length (factorize n)) 1) #t #f) 
  )
...
(define (cong-mod? m)
  "TODO: Implement"
  (lambda (a b)
    (if (equal? (remainder a m) (remainder b m)) #t #f)))
