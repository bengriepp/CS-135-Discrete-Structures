...
(define (indegree E v)
  "TODO: Implement"
(if (equal? E '()) 0
    (if (equal? (car (cdr (car E))) v) (+ 1 (indegree (cdr E) v))
        (+ 0 (indegree (cdr E) v))
    )
  ))
...
(define (find-topH G V)
(if (equal? G '()) (topsort-error)
    (if (equal? (indegree (get-edges V) (car G)) 0) (car G)
        (find-topH (cdr G) V))
  ))
...
(define (topsort G)
  "TODO: Implement"
(if (equal? (get-vertices G) '()) '()
    (append (list (find-top G)) (topsort (pop-vertex G (find-top G))))
  ))