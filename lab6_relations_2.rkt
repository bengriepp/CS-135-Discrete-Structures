...
(define (composeH x L)
  (if (null? L) '()
  (append (list (append (list x) (list (car L)))) (composeH x (cdr L)))))

(define (compose S R)
  "TODO: Implement"
   (if (null? R) '()
      (if (null? S) '()
          (if (not (null? (relate (cadar R) S))) (make-set (append (composeH (caar R) (relate (cadar R) S)) (compose S (cdr R)))) (compose S (cdr R))))))
...
(define (transitive-closure R)
  "TODO: Implement"
  (let loop ((count 0))
    (if (set-equal? (tcH count R) (tcH (+ 1 count) R))
        (tcH count R)
        (loop (+ count 1))))
  )
...
(if (equal? n 0) #t
(if (element? (surjectiveH? R) n)
    (surjective? (- n 1) R) #f
)))
(define (surjectiveH? R)
(if (null? R) '()
    (append (list (car (cdr (car R)))) (surjectiveH? (cdr R))))
)
